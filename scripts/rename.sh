...
NAME=$(echo ${TITLE//[[:blank:]]/} | iconv -t ascii//TRANSLIT | sed -E 's/[^a-zA-Z0-9-]+//g')
SLUG=$(echo $TITLE | iconv -t ascii//TRANSLIT | sed -E 's/ /-/g' | sed -E 's/[~\^]+//g' | sed -E 's/[^a-zA-Z0-9-]+//g' | sed -E 's/^-+\|-+$//g' | sed -E 's/\-+/-/g' | tr A-Z a-z)
...
echo ""
echo "Display app name: ${GREEN_COLOR}$TITLE${NO_COLOR} [current one is ${CYAN_COLOR}$CURRENT_APP_DISPLAY_NAME${NO_COLOR}]"
...
if [[ $RenamingResult = *"not a valid name"* ]]; then
...
if [ "$CURRENT_ANDROID_PACKAGE_ID_PATH" != "$PACKAGE_ID_PATH" ]; then
...
do
  if [ -f $file ]; then
    ...
    perl -pi -e "s/$CURRENT_APP_DISPLAY_NAME/$TITLE/g" $file
...
echo "${GREEN_COLOR}Done patching other files${NO_COLOR}"
...
perl -pi -e "s/$CURRENT_APP_DISPLAY_NAME/$TITLE/" ios/*/Info.plist

echo "${GREEN_COLOR}APP SUCCESSFULLY RENAMED TO \"$TITLE\"!${NO_COLOR}"